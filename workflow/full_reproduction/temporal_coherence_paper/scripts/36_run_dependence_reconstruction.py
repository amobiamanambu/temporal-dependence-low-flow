#!/usr/bin/env python3
"""Run the dependence-reconstruction and uncertainty analyses.
...
BASE_OUTPUT = PAPER_ROOT / "dependence_reconstruction"
...
def assemble(output_root: Path, inventory: pd.DataFrame, config: dict) -> dict:
    ...
    files = sorted(by_basin.glob("*_dependence_metrics.csv.gz"))
    if not files:
        raise RuntimeError("No completed dependence-reconstruction basin files found")
    frames = [pd.read_csv(path, dtype={"GAGE_ID": str}) for path in files]
    metrics = pd.concat(frames, ignore_index=True)
    metrics["GAGE_ID"] = normalize_gage(metrics["GAGE_ID"])
    atomic_csv(metrics, output_root / "dependence_reconstruction_metrics.csv.gz",
               compression="gzip")
    ...
    sample_rows = [
        ...
        ), "Completed dependence-reconstruction analysis"],
    ]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-label",
        default="full",
        help="Subfolder under dependence_reconstruction (default: full)",
    )
...
def main() -> None:
    ...
    for row in inventory.to_dict("records"):
        gage = str(row["GAGE_ID"]).zfill(8)
        output = by_basin / f"{gage}_dependence_metrics.csv.gz"

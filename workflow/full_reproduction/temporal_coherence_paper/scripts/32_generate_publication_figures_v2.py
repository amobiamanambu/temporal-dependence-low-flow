#!/usr/bin/env python3
"""Create the revised, information-dense figure suite for the Journal of Hydrology manuscript.
...
FIGURE_ROOT = PAPER_ROOT / "figures_v2"
...
RELEASE_ROOTS = (
    PAPER_ROOT / "data_release" / "v1" / "data",
    PAPER_ROOT / "zenodo_deposit" / "v1.0.0_upload",
)
...
def joined_curve(reference: str, score: str, old: pd.DataFrame,
    ...
    interval_path = REVIEW_ROOT / "archived_stratified_basin_intervals.csv"
    if not interval_path.exists():
        raise FileNotFoundError(
            "Run 36_run_reviewer_strengthening.py before generating intervals"
        )
...
def figure_3(
    ...
    review_path = REVIEW_ROOT / "reconstruction_comparisons.csv"
    if not review_path.exists():
        raise FileNotFoundError(
            "Run 36_run_reviewer_strengthening.py before generating Figure 3"
        )
    ...
    for index, (axis, score) in enumerate(zip(axes.flat, CORE)):
        ...
        if coherent.empty or reconstructed.empty:
            raise RuntimeError(f"Reviewer-strengthening results missing for {score}")
        data = coherent.copy()
        data["seasonal_reconstruction_score"] = reconstructed[
            "candidate_score"
        ].to_numpy(float)
        data["manuscript_score"] = score
        records.append(data)
...
def figure_s9() -> None:
    ...
    path = REVIEW_ROOT / "reconstruction_comparisons.csv"
    if not path.exists():
...
def figure_s10() -> None:
    ...
    comparison_path = REVIEW_ROOT / "reconstruction_comparisons.csv"
    loo_path = REVIEW_ROOT / "leave_one_water_year_out.csv"
    ...
    records = []
    for index, (axis, manuscript_score) in enumerate(zip(axes.flat, CORE)):
        score = score_map[manuscript_score]
        ...
        axis.axvline(97.5, color=CLASSIC_GRAY, lw=0.65, ls=":")
        axis.set_title(OUTCOME_LABELS[manuscript_score], loc="left")
